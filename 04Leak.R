...
### transform date
# people[,date := as.Date(as.character(date), format = "%Y-%m-%d")]
...
act_test$outcome <- NA
activs <- rbind(act_train,act_test)
#rm(TestActivs)
...
d1 <- merge(activs, people, by = "people_id", all.x = T)

# Remember, remember the 5th of November and which is test
testset <- which(people$people_id %in% d1$people_id[is.na(d1$outcome)])
...
allCompaniesAndDays <- data.table(
  expand.grid(unique(
    d1$group_1[!d1$people_id %in% people$people_id[testset]]), alldays
  )
)
...
meanbycomdate <- d1[
  !d1$people_id %in% people$people_id[testset],
  mean(outcome),
